...
function newComment(url, request) {
 ...
 if (requestComment && requestComment.body && requestComment.articleId &&
   ...
   if(!comment.username || !comment.articleId || !comment.body){
         response.status = 400;
   }
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    response.status = 400; // In your requirements you say "If no ID or updated comment is supplied, returns 200 response"
